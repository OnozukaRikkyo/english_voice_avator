...
# ナレーションの分割。N = ≤N 文字のパートに自然な切れ目で分割させる。
# -1 にすると分割せず1本にまとめる。
#
# 当初は heygen が「1パート = 1動画」で処理するための分割だったが、
# **台本の網羅性を保つ役割のほうが大きい**ことが実測で判明した。
#
# 同じ transcript（38,400字）での実測:
#     分割なし(-1)  9,823字(26%) / 17,244字(45%) / 11,774字(31%)  ← ぶれる
#     分割あり(7000)                19,598字(51%)                  ← 安定
#
# 分割すると各パートが個別に十分な長さで書かれるが、分割なしだと
# モデルが1回の応答で全体を書こうとして圧縮する。入力が長いほど圧縮率が上がる。
# プロンプトは「要約するな・全網羅せよ」と要求しているので、これは指示違反である。
#
# 1本の台本が欲しい場合も、分割したうえで concat_narration が
# 単一の SSML にまとめるので不都合はない。
